...
- **No args** — sweep cwd downward (maxdepth 4), same glob as `handup`
...
```bash
find . -maxdepth 4 \( -name "HANDOFF.*.yaml" \) 2>/dev/null \
  | grep -v -E "(target/|\.git/|node_modules/|\.ctx/)" | sort
```

For each file found:

- Read the `items` list and `log`
- Note the project name (from `project:` field or filename prefix)
- Read `.ctx/HANDOFF.state.yaml` alongside if present — extract `branch`, `build`, `tests`, `notes`
- Note the repo root (nearest ancestor containing `.git`)

If named-project scope was given, discard files whose `project:` field does not match.
...
**State-file notes not yet in extra**: If `.ctx/HANDOFF.state.yaml` has a `notes` field with
...
### 6. Update .ctx/HANDOFF.state.yaml notes (conditional)

If the analysis surfaced a new recommended focus or a newly resolved blocker that materially
changes the project's next action, update the `notes` field in `.ctx/HANDOFF.state.yaml`.
...
bash "$SCRIPT" --project <project-name> --handoff <path-to-handoff-yaml> 2>/dev/null
```
...
For each repo root that had at least one HANDOFF.yaml modified, stage and commit only the
HANDOFF file (not `.ctx/`):

```bash
git -C <repo-root> add HANDOFF.<project>.*.yaml
git -C <repo-root> commit -m "chore(handoff): handdown analysis pass $(date +%Y-%m-%d)"
```

If the working tree is dirty beyond the HANDOFF file, stage only the HANDOFF file explicitly.
Do not stage other changes.
