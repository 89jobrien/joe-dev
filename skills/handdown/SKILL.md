...
```bash
find . -maxdepth 5 -path "*/.ctx/HANDOFF.*.yaml" 2>/dev/null \
...
bash "$SCRIPT" upsert --project <project-name> --handoff <path-to-handoff-yaml> 2>/dev/null
```
...
```bash
git -C <repo-root> add .ctx/HANDOFF.<project>.*.yaml
